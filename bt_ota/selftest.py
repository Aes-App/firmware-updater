...
def test_block_payloads() -> None:
    req = bytes.fromhex("0001000000f0")
...
class _SimDevice:
    ...
    def __init__(self, emu: AuthEmulator):
        self.emu = emu
        self.rand = bytes([0x00]) + bytes(range(0x20, 0x30))
    ...
    def on(self, data: bytes):
        ...
        if data == AUTH_OK and not self._sent_challenge:
            self._sent_challenge = True
            return self.rand
        if len(data) == 17 and data[0] == 1:
...
def test_wiced() -> None:
    ...
    assert bytes([wiced.CMD_DOWNLOAD]) == b"\x01\x02"[1:]
    crc = 0xDEADBEEF
