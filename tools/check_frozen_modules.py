...
#: Every module the app reaches only through a lazy import, plus the generated
#: tables that have no import site at all outside their own package.
REQUIRED = [
    "bt_ota.gui",
    "bt_ota.update_check",
    "radio_contacts",
...
def _executable(path: str) -> str:
    """The Mach-O/PE that carries the archive, given a bundle or an exe."""
...
class Unreadable(Exception):
    """The build could not be inspected -- which is not the same as it being wrong."""
...
def frozen_modules(path: str) -> set:
    ...
    except Exception as e:  # noqa: BLE001 - any failure here means "cannot look"
    ...
    for entry in pyz_entries:
        ...
        try:
            ...
            os.unlink(tmp)
    # A onefile build can also carry loose .pyc entries in the CArchive itself.
