#!/usr/bin/env python3
"""Regenerate radio_contacts/contact_tables.py from the aes.app portal's tables.

The browser builder at aes.app/tools/digital_contact_download and this app must
put a contact in the same country, or the same user.csv yields two different
lists depending on which tool read it. So the tables are TRANSCRIBED from the
portal's own generated ones rather than retyped -- one chain, three
implementations, no hand copy.

Developer tool: it needs the portal checkout, which is not public. Nothing in
the app or the test suite runs it.

Run:  python3 tools/gen_contact_tables.py /path/to/portal
      python3 tools/gen_contact_tables.py --check /path/to/portal
"""
...
def js_object(src: str, name: str) -> dict:
    """The JS object literal assigned to `export const <name>` as a dict."""
...
def js_string_lists(src: str, name: str) -> dict:
    """A `const <name> = { KEY: ['A', 'B'], … }` block as {key: [values]}."""
