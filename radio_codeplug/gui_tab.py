"""The "Write Codeplug" tab.

Opened from the write dialog on cps.aes.app through an aesapp://codeplug link:
the web app has already built the write data, so this tab's whole job is to get
it onto the radio fast and honestly. Claim the link, show the operator what they
are about to overwrite, write it, verify it, and tell the server how it went.

Same threading shape as the other tabs: workers post to a queue the Tk thread
drains, so nothing touches a widget off the main thread.

Two things are deliberately NOT decided here. The codeplug's CONTENT is the web
app's business — this tab never edits or reinterprets the bytes. And the band
plan is not checked: the web app has already told the operator whether the
codeplug's plan and the radio's differ, and the radio itself rejects channels
outside its own plan, so a second refusal here would only be a dead end.
"""
...
class CodeplugWriteTab:
    ...
    def _refresh_write_state(self):
        ...
        for w in (self.link_btn, self.link_entry, self.port_box):
            ...
            except Exception:  # noqa: BLE001 — a readonly combobox refuses !disabled
                pass
    ...
    def handle_launch_url(self, url: str):
        """Entry point for an aesapp://codeplug link (OS handoff, forwarded
        instance, or the paste box). Claims the token on a worker thread."""
        try:
    ...
    def _write_worker(self, port, sess: client.CodeplugSession, job: client.CodeplugJob,
        ...
        def on_phase(phase, done, total):
            # Renewing the server lease. A dropped beat is not fatal (we keep
            # writing); the SERVER taking the lease away is, and heartbeat()
            # raises LockLostError for that.
            try:
    ...
    def _on_write_done(self, res: engine.WriteResult):
        ...
        # The lease is back either way; a fresh link is needed for another write.
        self.plan = None
        self.session_status.configure(
    ...
    # ---- asked by the main window before quitting ---------------------------
