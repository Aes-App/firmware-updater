"""The CBSEG1 container decoder: round trip, block iteration, and every way a
bad download must be refused (a short download must never become a short write).

Run:  python -m pytest tests/test_contacts_segments.py -q
"""
...
def _container(segs):
    """Hand-built container (independent of encode_container) per the doc."""
...
def test_round_trip_and_block_order():
    ...
    # encode_container is the exact inverse
...
@pytest.mark.parametrize("mutate, message", [
    ...
    (lambda raw: raw[:-1], "truncated"),                      # data cut short
    (lambda raw: raw[:12] + raw[12:16], "truncated header"),  # header cut short
    (lambda raw: raw + b"\x00", "trailing"),
    (lambda raw: raw[:8] + struct.pack(">I", 2) + raw[12:], "truncated"),   # count says 2, only 1 present
])
