"""Expected contact-write bytes, straight out of the server's byte-validated
encoders, for the synthetic lists the browser test builds too.

Developer tool: it regenerates tests/fixtures/contact-write-expected.json and
needs the server checkout that holds those encoders. The tests themselves need
nothing but the JSON it writes, so this is not run by the suite or by CI.

Run:  python3 gen-contact-write-expected.py /path/to/server/checkout
"""
import hashlib
import json
import os
...
# The encoders that were measured against the factory CPS. Give the path as the
# first argument or in $CONTACT_ENCODERS; there is deliberately no default.
...
from rdt_builder import contact_bundle as cb  # noqa: E402

ANYTONE_HEADER = ('"No.","Radio ID","Callsign","Name","City","State","Country",'
                  '"Remarks","Call Type","Call Alert"')

# The rows BOTH sides generate. Kept in one place here and mirrored verbatim in
# tests/js/digital-contacts.test.mjs.
COUNTRIES = [
    ("Canada", "CAN"),
    ("United States", "USA"),
    ("Germany", "DEU"),
    ("Türkiye", "TUR"),
    ("Ruritania", "RUR"),      # not in the country table -> first three letters
    ("", ""),                  # no country at all
]
# "\u00a0" is a NO-BREAK SPACE and nothing else. It is here because it is the one
# input that told three implementations apart: the CPS reads it as two
# unprintable bytes and stores "??", while a reader that trims with Python's own
# str.strip() -- or JavaScript's -- sees an empty city and shortens every record
# after it by two bytes. Two rows out of 296,967 in the live register carry it.
...
def digest(segments):
    """One hash over the whole write, plus a per-segment table."""
    h = hashlib.sha256()
    table = []
    blocks = 0
