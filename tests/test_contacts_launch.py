"""The aesapp:// link: parsing, the server allow-list, the Windows scheme
registration (winreg mocked) and the single-instance hand-off.

Run:  python -m pytest tests/test_contacts_launch.py -q
"""
...
@pytest.mark.parametrize("server, expect", [
    ...
    ("http://cps.aes.app", "https://cps.aes.app"),           # plain http to our host: not honoured
...
def test_windows_scheme_registration_is_idempotent():
    ...
    n = len(wr.created)
    # same path again: nothing rewritten
    assert launch.register_windows_scheme(r"C:\Apps\AesApp Radio Updater.exe", winreg_module=wr) is True
    assert len(wr.created) == n
    # a moved exe IS rewritten
...
def test_windows_registration_never_raises():
    ...
    assert launch.register_windows_scheme(None, winreg_module=FakeWinreg()) is False   # not frozen: no exe
...
def test_second_launch_forwards_its_link_to_the_running_instance(tmp_path):
    ...
    # not a link → refused, handler untouched
    import socket
