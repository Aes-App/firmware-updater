...
from radio_contacts import engine, segments as seg

IDENT_878 = b"ID878UV2" + b"\x0e" + b"V101" + b"\x00\x00" + b"\x06"
IDENT_7X2 = bytes.fromhex("49444d522d375832005631303001000606"[:32])
...
class PcModeDevice:
    ...
    def __init__(self, ident=IDENT_878, program_reply=b"QX\x06", drop_ack_at=(), stray_before=(),
        ...
        self.drop_ack_at = set(drop_ack_at)
        self.stray_before = set(stray_before)
...
def test_bootloader_reply_is_refused_and_nothing_is_written(fake_port):
    dev = PcModeDevice(program_reply=b"\x06")
    fake = fake_port(dev)
    lines, log = _logs()
